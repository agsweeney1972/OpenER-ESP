...
/*
 * Static-IP variant of app_main()
 *
 * Rename this file to main.c (and archive the DHCP-based main.c) to build
 * firmware that disables the Ethernet DHCP client and applies the fixed IP
 * information configured below.
 */

static const char *TAG = "opener_static_main";
                                ...
                                int32_t event_id, void *event_data)
{
    ...
    ESP_LOGI(TAG, "Ethernet Using Static IP Address");
    ESP_LOGI(TAG, "~~~~~~~~~~~");
...
void app_main(void)
{
    ...
    IP4_ADDR(&ip_info.ip,      192, 168, 1, 101); /* <-- adjust to your network */
    ...
    IP4_ADDR(&dns_info.ip.u_addr.ip4, 8, 8, 8, 8); /* primary DNS */
    ESP_ERROR_CHECK(esp_netif_set_dns_info(eth_netif, ESP_NETIF_DNS_MAIN, &dns_info));

    IP4_ADDR(&dns_info.ip.u_addr.ip4, 1, 1, 1, 1); /* secondary DNS */
 ...
 * Static-IP variant of app_main()
 *
 * Rename this file to main.c (and optionally archive the DHCP-based main.c)
 * to build firmware that disables the Ethernet DHCP client and applies the
 * fixed IP information configured below.
 */
